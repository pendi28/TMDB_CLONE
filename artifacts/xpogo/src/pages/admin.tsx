...
import { getToken, setToken, clearToken, getWriteToken, setWriteToken, clearWriteToken } from "@/lib/auth";
...
import {
  ...
  Play, Copy, CheckCheck, LayoutDashboard,
  ChevronUp, GripVertical,
  AlertTriangle, Key, Database,
} from "lucide-react";
...
type NavTab = "daftar" | "tambah" | "server" | "iklan" | "layout" | "tema" | "build";
...
function IklanTab() {
  ...
  const handleErr = (e: unknown) => setErrMsg(e instanceof Error ? e.message : String(e));

  const addAd = useMutation({
    mutationFn: () => fb.addAd(form),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["ads"] }); setShowForm(false); setForm(emptyAd()); setEditId(null); setErrMsg(""); },
    onError: handleErr,
  });
  ...
  const deleteAd = useMutation({ mutationFn: (id: string) => fb.deleteAd(id), onSuccess: () => qc.invalidateQueries({ queryKey: ["ads"] }), onError: handleErr });
  const toggleActive = useMutation({ mutationFn: ({ id, active }: { id: string; active: boolean }) => fb.updateAd(id, { active }), onSuccess: () => qc.invalidateQueries({ queryKey: ["ads"] }), onError: handleErr });

  const filtered = sub === "semua" ? ads : ads.filter((a) => a.type === sub);
  const openEdit = (ad: Ad) => { setEditId(ad.id); setForm({ type: ad.type, label: ad.label, code: ad.code, active: ad.active, platform: ad.platform ?? "both", delay: ad.delay ?? 0, frequency: ad.frequency ?? 1, adMobId: ad.adMobId ?? "", size: ad.size ?? "responsive" }); setShowForm(true); setErrMsg(""); };
  const openNew = (type?: AdType) => { setEditId(null); setForm({ ...emptyAd(), type: type ?? "banner-top" }); setShowForm(true); setErrMsg(""); };
  ...
  return (
    <div className="pb-4 space-y-5">
      ...
      {/* Warning jika tidak ada write token */}
      {!hasWriteToken && (
        <div className={`${card} p-3 border-orange-500/40 bg-orange-500/5 flex items-start gap-2`}>
          <AlertTriangle size={14} className="text-orange-400 flex-shrink-0 mt-0.5" />
          <div>
            <p className="text-orange-400 text-xs font-bold">Firebase Secret belum diset</p>
            <p className="text-gray-500 text-[10px] mt-0.5">Logout dan login ulang, lalu masukkan Firebase DB Secret agar bisa menyimpan iklan.</p>
          </div>
        </div>
      )}

      {errMsg && (
        <div className={`${card} p-3 border-red-500/40 bg-red-500/5 text-red-400 text-xs`}>
          ⚠️ {errMsg}
        </div>
      )}
      ...
      <div className={`${card} p-3 bg-blue-500/5 border-blue-500/20`}>
        <p className="text-blue-400 text-xs font-bold mb-1">📡 Sinkronisasi Otomatis Web + APK</p>
        <p className="text-gray-500 text-[10px]">Iklan yang diatur di sini otomatis tersinkron ke website dan APK melalui Firebase. Gunakan field <span className="text-white font-mono">Platform</span> untuk mengatur iklan khusus Web, APK, atau Keduanya.</p>
      </div>
      ...
      {showForm && (
        <div className={`${card} p-5 space-y-4`}>
          <div className="flex items-center justify-between mb-2">
            ...
            <button onClick={() => { setShowForm(false); setEditId(null); setForm(emptyAd()); setErrMsg(""); }} className="text-gray-500 hover:text-white"><X size={18}/></button>
          </div>
          ...
          <Field label="Platform">
            ...
            <p className="text-gray-500 text-[10px] mt-1">{AD_META[form.type]?.platformNote}</p>
          </Field>
          ...
          {errMsg && <p className="text-red-400 text-xs bg-red-500/10 border border-red-500/20 rounded-lg px-3 py-2">⚠️ {errMsg}</p>}
                ...
                <div className="flex items-start gap-3">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap mb-1">
                      ...
                      {!ad.active && <span className="px-2 py-0.5 rounded-md text-[10px] font-bold bg-gray-500/10 text-gray-500 border border-gray-500/20">NONAKTIF</span>}
                    </div>
                    <p className="text-white text-sm font-bold truncate">{ad.label || "(tanpa nama)"}</p>
                    <p className="text-gray-600 text-[10px] truncate font-mono mt-0.5">{ad.code.slice(0, 60)}{ad.code.length > 60 ? "…" : ""}</p>
                  </div>
...
/* ══════════════════════════════════════════════════════════ */
/* LAYOUT EDITOR TAB — wraps LayoutEditor component          */
/* ══════════════════════════════════════════════════════════ */
function LayoutTab() {
  return <LayoutEditor />;
}
...
function LoginPage({ onLogin }: { onLogin: () => void }) {
  ...
  const handle = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!secret.trim()) {
        setErr("Firebase DB Secret wajib diisi agar bisa menyimpan data.");
        return;
      }
      setToken(hash);
      setWriteToken(secret.trim());
      onLogin();
    } catch (ex) {
      setErr("Gagal login: " + String(ex));
    } finally {
      setLd(false);
    }
  };
      ...
      <div className={`${card} w-full max-w-sm p-8`}>
        ...
        <form onSubmit={handle} className="space-y-4">
          <Field label="Password Admin">
            <div className="relative">
              ...
              <input type={showPw ? "text" : "password"} value={pw} onChange={(e) => setPw(e.target.value)} placeholder="Masukkan password" autoFocus className={`${inp} pl-10 pr-10`} />
          ...
          <Field label="Firebase DB Secret">
            <div className="relative">
              <Key className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-500" />
              <input type={showSecret ? "text" : "password"} value={secret} onChange={(e) => setSecret(e.target.value)} placeholder="Firebase Database Secret key" className={`${inp} pl-10 pr-10 font-mono text-xs`} />
            ...
            <button type="button" onClick={() => setShowSecretHelp(!showSecretHelp)} className="text-[10px] text-blue-400 underline mt-1">
              {showSecretHelp ? "Tutup panduan" : "Di mana saya bisa dapatkan Secret ini?"}
            </button>
            {showSecretHelp && (
              <div className="mt-2 p-3 bg-blue-500/5 border border-blue-500/20 rounded-lg space-y-1">
                <p className="text-blue-400 text-[10px] font-bold">Cara mendapatkan Firebase DB Secret:</p>
                <ol className="text-gray-500 text-[10px] space-y-0.5 list-decimal pl-4">
                  <li>Buka <a href="https://console.firebase.google.com" target="_blank" rel="noreferrer" className="text-blue-400 underline">console.firebase.google.com</a></li>
                  <li>Pilih project Anda</li>
                  <li>Klik ⚙️ Project Settings → Service Accounts</li>
                  <li>Pilih tab <strong className="text-white">Database Secrets</strong></li>
                  <li>Klik "Show" pada secret yang ada, atau "Add Secret"</li>
                  <li>Copy dan paste di sini</li>
                </ol>
                <p className="text-yellow-400 text-[9px] mt-1">⚠️ Secret disimpan di sessionStorage browser saja, tidak dikirim ke server manapun.</p>
              </div>
            )}
          </Field>

          {err && <p className="text-red-400 text-xs bg-red-500/10 border border-red-500/20 rounded-lg px-3 py-2">{err}</p>}
          <button type="submit" disabled={loading || !pw || !secret} className={`${yaBtn} w-full flex items-center justify-center gap-2`}>
            {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : <Lock size={14} />}
        ...
        <div className="mt-6 p-3 bg-white/[0.03] rounded-xl border border-white/5">
          <p className="text-gray-600 text-[10px] text-center">
            ...
            Firebase Secret hanya disimpan di sesi browser ini, tidak pernah dikirim ke server lain.
          </p>
        </div>
      </div>
    </div>
...
function TemaTab() {
  ...
  const save = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["settings"] }); setSaved(true); setErrMsg(""); setTimeout(() => setSaved(false), 2500); },
  ...
  const changePw = async () => {
    ...
    } catch (ex) { setPwMsg({ text: "Gagal menyimpan: " + String(ex), ok: false }); }
  ...
  return (
    <div className="pb-8 space-y-5">
      ...
      <div className={card + " p-4 bg-blue-500/5 border-blue-500/20"}>
        ...
        <p className="text-gray-500 text-[11px] leading-relaxed">Server, iklan, dan pengaturan tersimpan di Firebase dan langsung tersinkron ke APK tanpa perlu rebuild. Iklan dengan platform "APK" atau "Keduanya" otomatis tampil di aplikasi Android.</p>
      </div>
      <div className={card + " p-4 bg-green-500/5 border-green-500/20"}>
        <p className="text-green-400 text-xs font-bold mb-2">✅ Mandiri dari Replit</p>
        <p className="text-gray-500 text-[11px] leading-relaxed">Website dan APK berjalan sepenuhnya via Firebase. Tidak ada ketergantungan dengan server Replit — aman dideploy ke mana saja (Firebase Hosting, Vercel, Netlify, dll).</p>
      </div>
    </div>
  );
}
...
/* ══════════════════════════════════════════════════════════ */
/* TAMBAH TAB (film/series)                                 */
/* ══════════════════════════════════════════════════════════ */
type TmdbItem = TmdbListItem & { imdb_id?: string };
type AddSubTab = "cari" | "id" | "pilihan";

const emptyFilm = { title: "", description: "", posterUrl: "", backdropUrl: "", year: "", embedUrl: "", type: "movie" as "movie" | "tv", tmdbId: "", imdbId: "" };
...
function TambahTab() {
  ...
  const [selectedServer, setSelectedServer] = useState("peachify");
  const formRef = useRef<HTMLDivElement>(null);

  function buildEmbedUrl(serverId: string, tmdbId: string, type: "movie" | "tv") {
    const srv = ALL_BUILTIN.find(s => s.id === serverId);
    if (!srv || !tmdbId) return "";
    const template = type === "movie" ? srv.movieUrl : srv.tvUrl;
    return resolveUrl(template, tmdbId);
  }
  ...
  const doSearch = async () => {
    ...
    setLd(true);
    try {
      const r = await tmdb.search(q, mediaType);
  ...
  const doFetchId = async () => {
    ...
    try {
      let item: TmdbItem;
      if (idInput.startsWith("tt")) {
        const res = await tmdb.findByImdb(idInput);
        const found = (res as TmdbFindResult).movie_results?.[0] ?? (res as TmdbFindResult).tv_results?.[0];
        if (!found) throw new Error("Not found");
        item = { ...found, media_type: (res as TmdbFindResult).movie_results?.length ? "movie" : "tv" };
      } else {
        ...
        item = {
          ...
          name: "name" in res ? (res as TmdbTvShow).name : undefined,
          overview: res.overview,
          poster_path: res.poster_path,
          backdrop_path: res.backdrop_path,
          release_date: "release_date" in res ? res.release_date : undefined,
          first_air_date: "first_air_date" in res ? (res as TmdbTvShow).first_air_date : undefined,
...
/* MAIN ADMIN                                               */
/* ══════════════════════════════════════════════════════════ */
const NAV: { id: NavTab; label: string; icon: React.ComponentType<{ size?: number }>; special?: boolean }[] = [
...
export default function AdminPage() {
  ...
  useEffect(() => {
    ...
    fb.getPasswordHash().then((hash) => {
      if (hash && token === hash) setLI(true);
      else { clearToken(); clearWriteToken(); }
    }).finally(() => setV(false));
  }, []);
  ...
  return (
    <div className="min-h-screen bg-[#0d1117] flex flex-col text-white">
      <div className="bg-[#0d1117] border-b border-white/[0.08] px-4 py-3 flex items-center justify-between sticky top-0 z-10">
        ...
        <button onClick={handleLogout} className="text-red-500 p-2 border border-red-500/20 rounded-xl hover:bg-red-500/10 transition-colors">
          <LogOut size={18}/>
        </button>
      </div>
      ...
      {/* Bottom nav — scrollable on small screens */}
      <div className="fixed bottom-0 left-0 right-0 bg-[#0d1117] border-t border-white/[0.08] z-20 overflow-x-auto" style={{ paddingBottom: "env(safe-area-inset-bottom, 0px)" }}>
        <div className="flex items-end justify-around min-w-max w-full px-2 pt-2 pb-3">
          {visibleNav.map((n) => (
            <button key={n.id} onClick={() => setTab(n.id)} className={`flex flex-col items-center px-3 transition-all ${tab === n.id ? "text-yellow-400 scale-110" : "text-gray-600"}`}>
              {n.special ? (
                <div className={`w-12 h-12 rounded-full flex items-center justify-center -mb-1 border-4 border-[#0d1117] ${tab === n.id ? "bg-yellow-400 text-black" : "bg-[#1e2535] text-white"}`}>
                  <n.icon size={24} />
                </div>
              ) : (
                <>
                  <n.icon size={18} />
                  <span className="text-[8px] font-bold mt-0.5 uppercase whitespace-nowrap">{n.label}</span>
                </>
              )}
            </button>
          ))}
