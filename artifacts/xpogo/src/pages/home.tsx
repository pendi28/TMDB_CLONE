    ...
    <div className="relative group">
      ...
      <div ref={rowRef} className="flex gap-3 overflow-x-auto scrollbar-hide pb-2 pl-1 pr-1">
        {items.map((item) => {
          ...
          return (
            <MovieCard key={item.id} id={item.id} title={title}
              posterPath={item.poster_path} rating={item.vote_average}
              year={year} mediaType={type} />
          );
        })}
      </div>
...
function LangBadge({ lang }: { lang?: string }) {
  ...
  const m = lang ? map[lang] : null;
  if (!m) return null;
  return (
    <span className="text-[9px] font-black px-1.5 py-0.5 rounded text-white" style={{ backgroundColor: m.color }}>
      {m.label}
    </span>
  );
}
      ...
      <div className="relative flex flex-col sm:flex-row items-start sm:items-center gap-5 p-5">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-2">
            <h2 className="text-white text-xl sm:text-2xl font-black leading-tight">{title}</h2>
            <LangBadge lang={lang} />
          </div>
...
function SectionHeader({ title, badge, badgeColor }: { title: string; badge?: string; badgeColor?: string }) {
  return (
    <div className="flex items-center gap-2 mb-3">
      ...
      {badge && (
        <span className="text-[9px] font-black px-2 py-0.5 rounded text-white" style={{ backgroundColor: badgeColor ?? "#E50914" }}>
          {badge}
        </span>
      )}
    </div>
...
function SearchableHeader({ title }: { title: string }) {
...
export default function HomePage() {
  ...
  const { data: dramaTaiwan }      = useQuery<TmdbListResult>({ queryKey: ["drama-taiwan"],      queryFn: () => tmdb.dramaTaiwan() });
  const { data: dramaThailand }    = useQuery<TmdbListResult>({ queryKey: ["drama-thailand"],    queryFn: () => tmdb.dramaThailand() });
  const { data: dramaIndonesia }   = useQuery<TmdbListResult>({ queryKey: ["drama-indonesia"],   queryFn: () => tmdb.dramaIndonesia() });
  ...
  const { data: filmThailand }  = useQuery<TmdbListResult>({ queryKey: ["film-thailand"],  queryFn: () => tmdb.filmThailand() });
  const { data: bollywood }     = useQuery<TmdbListResult>({ queryKey: ["bollywood"],      queryFn: () => tmdb.bollywood() });
  const { data: filmIndonesia } = useQuery<TmdbListResult>({ queryKey: ["film-indonesia"], queryFn: () => tmdb.filmIndonesia() });
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-16 -mt-8 relative z-10">
        ...
        {(donghuaNew?.results?.length ?? 0) > 0 && (
          <section className="mb-8">
            ...
            <HorrorCarousel items={donghuaNew!.results} mediaType="tv" />
          </section>
        )}
        {(donghua?.results?.length ?? 0) > 0 && (
          <section className="mb-8">
            <SectionHeader title="🐉 DONGHUA TERPOPULER" badge="DONGHUA" />
            <HorrorCarousel items={donghua!.results} mediaType="tv" />
          </section>
        )}
        {(donghuaTopRated?.results?.length ?? 0) > 0 && (
          <section className="mb-8">
            <SectionHeader title="⭐ DONGHUA TOP RATING" badge="TOP" badgeColor="#f59e0b" />
            <HorrorCarousel items={donghuaTopRated!.results} mediaType="tv" />
          </section>
        )}
        ...
        {(animeNew?.results?.length ?? 0) > 0 && (
          <section className="mb-8">
            <SectionHeader title="🆕 ANIME TERBARU" badge="NEW" badgeColor="#7c3aed" />
            <HorrorCarousel items={animeNew!.results} mediaType="tv" />
          </section>
        )}
        {(anime?.results?.length ?? 0) > 0 && (
          <section className="mb-8">
            <SectionHeader title="⛩️ ANIME JEPANG POPULER" badge="ANIME" badgeColor="#7c3aed" />
            <HorrorCarousel items={anime!.results} mediaType="tv" />
          </section>
        )}
        {(animeTopRated?.results?.length ?? 0) > 0 && (
          <section className="mb-8">
            <SectionHeader title="🏆 ANIME TOP RATING" badge="TOP" badgeColor="#7c3aed" />
            <HorrorCarousel items={animeTopRated!.results} mediaType="tv" />
          </section>
        )}
        ...
        {(dramaKoreaNew?.results?.length ?? 0) > 0 && (
          <section className="mb-8">
            <SectionHeader title="🆕 K-DRAMA TERBARU" badge="NEW" badgeColor="#0369a1" />
            <HorrorCarousel items={dramaKoreaNew!.results} mediaType="tv" />
          </section>
        )}
        {(dramaKorea?.results?.length ?? 0) > 0 && (
          <section className="mb-8">
            <SectionHeader title="🇰🇷 DRAMA KOREA POPULER" badge="K-DRAMA" badgeColor="#0369a1" />
            <HorrorCarousel items={dramaKorea!.results} mediaType="tv" />
          </section>
        )}
        {(filmKorea?.results?.length ?? 0) > 0 && (
          <section className="mb-8">
            <SectionHeader title="🎬 FILM KOREA" badge="K-MOVIE" badgeColor="#0369a1" />
            <HorrorCarousel items={filmKorea!.results} mediaType="movie" />
          </section>
        )}
        ...
        {(dramaChina?.results?.length ?? 0) > 0 && (
          <section className="mb-8">
            <SectionHeader title="🇨🇳 CHINESE DRAMA" badge="C-DRAMA" />
            <HorrorCarousel items={dramaChina!.results} mediaType="tv" />
          </section>
        )}
        {(dramaTaiwan?.results?.length ?? 0) > 0 && (
          <section className="mb-8">
            <SectionHeader title="🇹🇼 TAIWAN DRAMA" badge="TW" badgeColor="#dc2626" />
            <HorrorCarousel items={dramaTaiwan!.results} mediaType="tv" />
          </section>
        )}
        {(filmChina?.results?.length ?? 0) > 0 && (
          <section className="mb-8">
            <SectionHeader title="🎬 FILM CHINA" badge="C-MOVIE" />
            <HorrorCarousel items={filmChina!.results} mediaType="movie" />
          </section>
        )}
        ...
        {(dramaThailand?.results?.length ?? 0) > 0 && (
          <section className="mb-8">
            <SectionHeader title="🇹🇭 DRAMA THAILAND" badge="THAI" badgeColor="#d97706" />
            <HorrorCarousel items={dramaThailand!.results} mediaType="tv" />
          </section>
        )}
        {(filmThailand?.results?.length ?? 0) > 0 && (
          <section className="mb-8">
            <SectionHeader title="🎬 FILM THAILAND" badge="THAI" badgeColor="#d97706" />
            <HorrorCarousel items={filmThailand!.results} mediaType="movie" />
          </section>
        )}
        {(dramaJapan?.results?.length ?? 0) > 0 && (
          <section className="mb-8">
            <SectionHeader title="🇯🇵 J-DRAMA" badge="JDRAMA" badgeColor="#7c3aed" />
            <HorrorCarousel items={dramaJapan!.results} mediaType="tv" />
          </section>
        )}
        {(filmJepang?.results?.length ?? 0) > 0 && (
          <section className="mb-8">
            <SectionHeader title="🎬 FILM JEPANG" badge="J-MOVIE" badgeColor="#7c3aed" />
            <HorrorCarousel items={filmJepang!.results} mediaType="movie" />
          </section>
        )}
        ...
        {(dramaIndonesia?.results?.length ?? 0) > 0 && (
          <section className="mb-8">
            <SectionHeader title="🇮🇩 DRAMA INDONESIA" badge="INDO" badgeColor="#059669" />
            <HorrorCarousel items={dramaIndonesia!.results} mediaType="tv" />
          </section>
        )}
        {(filmIndonesia?.results?.length ?? 0) > 0 && (
          <section className="mb-8">
            <SectionHeader title="🎬 FILM INDONESIA" badge="LOKAL" badgeColor="#059669" />
            <HorrorCarousel items={filmIndonesia!.results} mediaType="movie" />
          </section>
        )}
        {(dramaPhilippines?.results?.length ?? 0) > 0 && (
          <section className="mb-8">
            <SectionHeader title="🇵🇭 DRAMA FILIPINA" badge="PH" badgeColor="#2563eb" />
            <HorrorCarousel items={dramaPhilippines!.results} mediaType="tv" />
          </section>
        )}
        {(bollywood?.results?.length ?? 0) > 0 && (
          <section className="mb-8">
            <SectionHeader title="🎬 BOLLYWOOD" badge="INDIA" badgeColor="#dc2626" />
            <HorrorCarousel items={bollywood!.results} mediaType="movie" />
          </section>
        )}
