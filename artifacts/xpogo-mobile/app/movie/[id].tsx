...
function buildPeachifyMovieUrl(id: number) {
  return `https://peachify.top/embed/movie/${id}?accent=E50914&autoplay=1`;
}
...
export default function MovieDetailScreen() {
  ...
  const loadFirebaseData = useCallback(async () => {
    await fb.getEmbeds().then((list: any[]) => {
      setEmbeds(list.filter(e => e.active && e.type === "movie" && e.tmdbId === movieId));
    }).catch(() => {});
  }, [movieId]);
  ...
  const m = movie;
  const title = m?.title ?? m?.original_title ?? "Film";
  const originalName = m?.original_title ?? title;
  const origLang = m?.original_language ?? "";
  const overview = m?.overview ?? "";
  const backdropUrl = m?.backdrop_path ? `${IMG}/original${m.backdrop_path}` : "";
  const posterUrl = m?.poster_path ? `${IMG_W500}${m.poster_path}` : "";
    ...
    const srvs: ServerOption[] = [
      {
        ...
        url: `https://player.vidplus.to/embed/movie/${movieId}?primarycolor=E50914&secondarycolor=170000&iconcolor=FFFFFF&autoplay=true&icons=netflix`,
        badge: "NEW", badgeColor: "#6C63FF", icon: "🎬",
      },
      {
        id: "vidzee",
        label: "VidZee",
        url: `https://player.vidzee.wtf/embed/movie/${movieId}`,
      ...
      },
      {
        ...
        url: buildPeachifyMovieUrl(movieId),
  ...
  const openPlayer = (srv: ServerOption) => {
    ...
    router.push(
      `/player?url=${encodeURIComponent(urlParam)}&title=${encodeURIComponent(title)}&tmdbId=${movieId}&mediaType=movie&originalName=${encodeURIComponent(originalName)}` as never
    );
  };
    ...
    <View style={S.container}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 40 }}>
        ...
        <View style={S.infoRow}>
          ...
          <View style={S.metaCol}>
            ...
            <View style={S.metaRow}>
              ...
              {m?.runtime ? <Text style={S.metaChip}>⏱ {m.runtime} mnt</Text> : null}
            </View>
            {m?.vote_average ? <RatingCircle score={m.vote_average} /> : null}
          </View>
        </View>
        ...
        <View style={{ paddingHorizontal: 16, marginBottom: 20, gap: 10 }}>
          <TouchableOpacity style={S.btnWatchNow} onPress={() => openPlayer(servers[0])} activeOpacity={0.85}>
            <Text style={S.btnWatchNowText}>▶  Tonton Sekarang</Text>
          </TouchableOpacity>
          <TouchableOpacity style={S.btnServer} onPress={() => setPickerVisible(true)} activeOpacity={0.85}>
            <Text style={S.btnServerText}>🖥️  Pilih Server ({servers.length} tersedia)</Text>
          </TouchableOpacity>
          <TouchableOpacity style={S.btnShare} onPress={() => setShareVisible(true)} activeOpacity={0.85}>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false}
              ...
              {similar.map((s: any) => (
                <TouchableOpacity key={s.id} style={{ width: 90 }}
                  onPress={() => router.push(`/movie/${s.id}` as never)} activeOpacity={0.85}>
                  {s.poster_path
        ...
        <CommentsSection type="movie" tmdbId={movieId} />
      </ScrollView>
